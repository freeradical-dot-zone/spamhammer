...
DO $$
BEGIN
    RAISE NOTICE '% Dropping the constraint and recreating the function', CLOCK_TIMESTAMP();
END;
$$;
...
DECLARE
    ...
    -- match anything.
    abuse_text_strings TEXT ARRAY = ARRAY['%<a href="https://midokuriserver.github.io/minidon/%', '%black friday%shorturl.at%',
    ...
    '%<p>%<br />mastodon safety team<%', '%<p>%<br />mastodon security<%', '%<p>%<br />mastodon support team<%', '%<p>%<br />mastodon team<%', '%<p>%<br />mastodon user services<%', '%<p>%<br />the mastodon safety team<%', '%<p>%<br />the mastodon security team<%', '%<p>%<br />the mastodon support team<%', '%<p>%<br />the mastodon team<%', '%<p>%<br />the mastodon user services team<%', '%<p>%<br />— mastodon safety team<%', '%<p>%<br />— mastodon security<%', '%<p>%<br />— mastodon support team<%', '%<p>%<br />— mastodon team<%', '%<p>%<br />— mastodon user services team<%', '%<p>%<br />— the mastodon safety team<%', '%<p>%<br />— the mastodon security<%', '%<p>%<br />— the mastodon support team<%', '%<p>%<br />— the mastodon team<%', '%<p>%<br />— the mastodon user services team<%',
    -- Seriously, stahp
    ...
    '%https://mastodon.id-63384.world/%', '%https://mastodon.netprocesse.com/%', '%processkyc.com/%'];
BEGIN
...
DO $$
BEGIN
    RAISE NOTICE '% Running unit tests', CLOCK_TIMESTAMP();
END;
$$;
...
DECLARE
    ham_string TEXT;
    sample_ham_strings TEXT ARRAY = ARRAY['hello, world!', 'You can add me on Friendica: @me@friendica'];
BEGIN
    ...
    FOREACH ham_string IN ARRAY sample_ham_strings LOOP
        ASSERT (
            SELECT
                *
            FROM frz_text_is_abusive (ham_string)) = FALSE,
        'Matched a ham status: ''' || ham_string || '''';
    END LOOP;
END;
$$
LANGUAGE plpgsql;
...
DECLARE
    ...
    '<p><span class="h-card" translate="no"><a href="@someuser" class="u-url mention">@<span>someuser</span></a></span> <br />🚨 Automatic User Notification 🚨</p><p>Your account has been suspended. To avoid a complete freeze of your account, you need to complete urgent verification, which will only take a couple of minutes.</p><p>⏳ Time Limit: 30 Minutes  <br />🔍 Required Action: Finish verification using the link below.</p><p>If not completed, your account will remain locked until further evaluation.</p><p>🔗 Verification Link: <a href="https://approve-gig.com/1805649434" target="_blank" rel="nofollow noopener" translate="no"><span class="invisible">https://</span><span class="">approve-gig.com/1805649434</span><span class="invisible"></span></a></p><p>Sincerely,  <br />Mastodon Support Team</p>', '<p><span class="h-card" translate="no"><a href="@someuser" class="u-url mention">@<span>someuser</span></a></span> Your  account has been temporarily suspended due to uploaded material that appears to violate USA law.<br />To address this and potentially avoid administrative or criminal liability (including fines up to $3,000 or imprisonment for up to 2 years), we require you to verify your identity.</p><p>Please use the pdf below to complete your identity verification:<br /><a href="https://continued.short.gy/UDhn" target="_blank" rel="nofollow noopener" translate="no"><span class="invisible">https://</span><span class="">continued.short.gy/UDhn</span><span class="invisible"></span></a></p>', '<p>Access Privilege Update<br />Verification Level: Incomplete<br /> Your security clearance necessitates reapproval. Current rating: Below Requirements.<br />Obtain validation:<br />🔗 <a href="https://mastodon.netprocesse.com/mx/u/1852916528" target="_blank" rel="nofollow noopener noreferrer" translate="no"><span class="invisible">https://</span><span class="ellipsis">mastodon.netprocesse.com/mx/u/</span><span class="invisible">1852916528</span></a><br />Unauthorized accounts will experience reduced functionality.<br />Mastodon Security Administration</p>', '<p><span class="h-card" translate="no"><a href="@someuser" class="u-url mention">@<span>somuser</span></a></span> Hello,</p><p>Our records indicate that your account has not been verified yet. As part of our updated community standards, we now require all users to complete a brief verification step in order to retain full access to their profiles and associated content.</p><p>To verify your account, please use the following link:</p><p>🔗 <a href="https://mastodon.infprocess.com/mx/z/1764522627" target="_blank" rel="nofollow noopener" translate="no"><span class="invisible">https://</span><span class="ellipsis">mastodon.infprocess.com/mx/z/1</span><span class="invisible">764522627</span></a></p><p>The process is quick and should take less than a minute. Please note that unverified accounts may experience temporary access limitations.</p><p>We appreciate your understanding and cooperation,<br />The Mastodon Support Team</p>', '<p><span class="h-card" translate="no"><a href="@someuser" class="u-url mention">@<span>someuser</span></a></span> Alert: Verify Your Mastodon Account<br />We&#39;ve detected unusual activity. To keep your access, please verify your account now:<br />🔗 <a href="https://verify.form98441.icu/7S1C1I0N5P2" target="_blank" rel="nofollow noopener" translate="no"><span class="invisible">https://</span><span class="ellipsis">verify.form98441.icu/7S1C1I0N5</span><span class="invisible">P2</span></a><br />Copy and paste the link if it doesn&#39;t open.<br />Verification is quick and required to avoid suspension.<br />No action within 24h may lead to temporary lock.<br />— Mastodon Security<br />[Do not reply – automated message]</p>', '<p><span class="h-card" translate="no"><a href="@someuser" class="u-url mention">@<span>someuser</span></a></span>  Urgent: Complete Your Mastodon Verification</p><p>To ensure a safe experience for all, we&#39;re implementing mandatory account verification. Our records show we&#39;re still missing yours!</p><p>Quick verification:<br />🛡 Secure your account in 2 minutes:<br />🔗 [<a href="https://verify.form98441.icu/8T3F7X9X2B6" target="_blank" rel="nofollow noopener" translate="no"><span class="invisible">https://</span><span class="ellipsis">verify.form98441.icu/8T3F7X9X2</span><span class="invisible">B6</span></a>]</p><p>⏰ Time-sensitive: Restrictions apply to unverified accounts after *[current date + 48 hours]*.</p><p>Thank you for helping us build a safer community!<br />— Mastodon Safety Team</p>', '<p><span class="h-card" translate="no"><a href="@someuser" class="u-url mention">@<span>someuser</span></a></span> Action Required: Verify to Keep Access<br />Your account has not been verified under our new user policy requirements.<br />Please confirm your identity to maintain full access:<br />🔗 <a href="https://verify.form98441.icu/7S1C1I0N5P2" target="_blank" rel="nofollow noopener" translate="no"><span class="invisible">https://</span><span class="ellipsis">verify.form98441.icu/7S1C1I0N5</span><span class="invisible">P2</span></a><br />Unverified accounts will lose access.<br />Mastodon User Services</p>', '<p><span class="h-card" translate="no"><a href="@someuser" class="u-url mention">@<span>someuser</span></a></span> Action Required: Verify Your Mastodon Account</p><p>To enhance security and comply with our updated policies, we now require all users to complete identity verification. Our system shows that your account remains unverified.</p><p>Next Steps:<br />✅ Click below to complete verification now:<br />🔗 [<a href="https://verify.form98441.icu/8T3F7X9X2B6" target="_blank" rel="nofollow noopener noreferrer" translate="no"><span class="invisible">https://</span><span class="ellipsis">verify.form98441.icu/8T3F7X9X2</span><span class="invisible">B6</span></a>]</p><p>⚠ Please note: Unverified accounts may lose full access within 48 hours. Don’t risk disruptions—secure your account today.</p><p>We appreciate your cooperation!<br />— The Mastodon Team</p>', 'User,</p><p>Your account has been suspended due to content violating new EU legislation on age-restricted material. This law carries serious consequences, including significant fines, prosecution, and potential imprisonment. To comply and protect yourself, verify your identity immediately by clicking the link below. Failure to do so may result in permanent account closure and reporting to authorities.</p><p>🔺processkyc.com/1827996243</p><p>This is a legal requirement under EU law.</p>'];
BEGIN
    ...
    FOREACH spam_string IN ARRAY sample_spam_strings LOOP
        ASSERT (
            SELECT
                *
            FROM frz_text_is_abusive (spam_string)) = TRUE,
        'Did not match a spam status: ''' || spam_string || '''';
    END LOOP;
END;
$$
LANGUAGE plpgsql;
...
DO $$
BEGIN
    RAISE NOTICE '% Adding the constraint for new statuses', CLOCK_TIMESTAMP();
END;
$$;
...
DO $$
BEGIN
    RAISE NOTICE '% Deleting existing abusive statuses', CLOCK_TIMESTAMP();
END;
$$;

DELETE FROM statuses
WHERE frz_text_is_abusive (text);

DO $$
BEGIN
    RAISE NOTICE '% Validating the constrainst against existing statuses', CLOCK_TIMESTAMP();
END;
$$;
